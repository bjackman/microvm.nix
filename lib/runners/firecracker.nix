...
let
  ...
  inherit (microvmConfig)
    hostName user socket preStart vcpu mem balloon initialBalloonMem hotplugMem
    hotpluggedMem interfaces volumes shares devices kernel initrdPath storeDisk
    credentialFiles;
  ...
  baseConfig = {
    boot-source = {
      ...
      boot_args =
        "console=ttyS0,115200 noapic acpi=off reboot=k panic=1 i8042.noaux i8042.nomux i8042.nopnp i8042.dumbkbd ${
  ...
  command = if user != null then
    ...
    lib.escapeShellArgs ([
      ...
      (if socket != null then
        ...
        throw
        "Firecracker must be configured with an API socket (option microvm.socket)!")
